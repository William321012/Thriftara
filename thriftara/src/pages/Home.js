...
function Home() {
  ...
  const addCart = async (id, num) => {
    await axios.post(
      `http://localhost:8080/carts/insertToCart?pid=${id}&amount=${num}`
    );
  };
      ...
      <div className="home-container">
        ...
        <section className="mens">
          ...
          <section className="products-section">
            {products
              .filter((product) => product.gender === 1)
              .map((product) => (
        ...
        <section className="womens">
          ...
          <section className="products-section">
            {products
              .filter((product) => product.gender === 2)
              .map((product) => (
                <div className="product-items" key={product.id}>
                  {createImgURL(product.image)}
                  <img
        ...
        <section className="recommendations">
          ...
          <div className="rec-container">
            {products
              .filter((product) => product.priority < 5)
              .map((product) => (
